import { decrypt_message, decrypt_message_with_bytes, decrypt_with_secret_key, debug_can_decrypt, EncryptedMessage, PrivateKey } from 'cipher';
...
export class CipherHelper {
  ...
  static error(...args: any[]): void {
    if (args[0]?.includes("Failed to decrypt with both receive and change keys") ||
  ...
  static ensureWasmInitialized(): boolean {
    try {
      ...
      if (typeof EncryptedMessage !== 'function' || typeof PrivateKey !== 'function') {
   ...
   */
  static async tryDecrypt(encryptedHex: string, privateKeyHex: string, messageId: string): Promise<string> {
    // Validate inputs
    if (!encryptedHex || !privateKeyHex) {
      throw new Error('Invalid input: encrypted message and private key are required');
    }

    // Check rate limiting and attempt tracking
    if (!SecurityHelper.canAttemptDecryption(messageId)) {
      throw new Error('Decryption attempts rate limited or maximum attempts reached');
    }
    ...
    try {
      ...
      const decrypted = await decrypt_with_secret_key(encryptedMessage, privateKeyBytes);
    ...
    const errorDetails = errors.map((err, i) => `Method ${i+1}: ${err.message || err}`).join("; ");
